...
async function fetchQueueState(
  token: string,
  passes: MutableRefObject<Set<string>>
): Promise<Queue> {
        ...
        >
          <Toast.Header>
            <strong className="me-auto">
              {servers.find((server) => server.port == event.serverPort)?.name}
            </strong>
...
function Home({
  ...
  const [serversStatus, setServersStatus] =
    useState<ServersStatus>(initialServers);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      const profileData = await requestBackendData(
        "/api/v1/auth/profile",
        token
      );
  ...
  return (
    <Container className="p-0" fluid>
      ...
      <CommonNavBar
        isAdmin={
          profile != null &&
          hasFlag(profile, AdminFlag.R_ADMIN) &&
          hasFlag(profile, AdminFlag.R_SERVER)
        }
        token={token}
      />

      {PassToasts(serversStatus.servers, passEvents, (event) => {
